...
class DAS_PW(torch.nn.Module):
    ...
    def __init__(
        ...
        elif not hasattr(ang_list, "__getitem__"): # hasattr用于判断对象是否包含对应的属性。
            ang_list = [ang_list]
        if ele_list is None:
        ...
        self.out_shape = grid.shape[:-1] #图片大小508*387
    ...
    def forward(self, x):
        ...
        for t, td, ta in tqdm(zip(self.ang_list, txdel, txapo), total=nangles):
            for r, rd, ra in zip(self.ele_list, rxdel, rxapo):
                ...
                # Apply phase-rotation if focusing demodulated data 相位旋转
                if self.fdemod != 0:
...
## Simple phase rotation of I and Q component by complex angle theta 相位旋转函数
...
def delay_plane(grid, angles):
    # Use broadcasting to simplify computations 在最开始增加一个维度
    x = grid[:, 0].unsqueeze(0)
    z = grid[:, 2].unsqueeze(0)
    # For each element, compute distance to pixels 计算延迟距离
...
def apod_plane(grid, angles, xlims): # 发射偏迹
