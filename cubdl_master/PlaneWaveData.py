...
class PICMUSData(PlaneWaveData):
    ...
    def __init__(self, database_path, acq, target, dtype):
        ...
        assert any([target == t for t in ["contrast_speckle", "resolution_distorsion","carotid_cross","carotid_long"]])
        ...
        f1 = h5py.File(fname1, "r")["US"]["US_DATASET0000"]
        ...
        self.x_axis = np.array(f1["x_axis"], dtype="float32")
        self.z_axis = np.array(f1["z_axis"], dtype="float32")
